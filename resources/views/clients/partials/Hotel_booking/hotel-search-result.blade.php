...
@foreach($hotels as $hotel)
            <div class="card-item card-item-list">
              <div class="card-img">
                <a href="/hotel-single?hotel_id={{ $hotel->id }}" class="d-block">
                  <img src="{{ $hotel->cover_image ? asset('storage/' . $hotel->cover_image) : asset('ui/images/img1.jpg') }}" alt="hotel-img" />
                </a>
                @if($hotel->is_featured ?? false)
              ...
              <div class="card-body">
                <h3 class="card-title">
                  <a href="/hotel-single?hotel_id={{ $hotel->id }}">{{ $hotel->name }}</a>
                </h3>
                <p class="card-meta">{{ $hotel->address }}</p>
                <div class="card-rating">
                  <span class="badge text-white">{{ $hotel->rating ?? '4.4/5' }}</span>
                  <span class="review__text">Average</span>
                  <span class="rating__text">({{ $hotel->reviews_count ?? '30' }} Reviews)</span>
                </div>
                <div class="card-price d-flex align-items-center justify-content-between">
                  <p>
                    <span class="price__from">From</span>
                    <span class="price__num">${{ $hotel->price ?? '88.00' }}</span>
                    <span class="price__text">Per night</span>
                  </p>
                  <a href="/hotel-single?hotel_id={{ $hotel->id }}" class="btn-text">See details<i class="la la-angle-right"></i></a>
                </div>
              </div>
            </div>
@endforeach
